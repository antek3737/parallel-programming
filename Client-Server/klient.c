...
void sending(struct message *toSend){
    int ret;
    do {
        ret = msgsnd(IDqueue, toSend, MAX + sizeof(long int), IPC_NOWAIT);
        if(ret != -1){
            return;
        }
        sleep(3);
...
void *receive(void *ptr)
{
  ...
  while (2)
  {
    if (msgrcv(IDqueue, &received, MAX + sizeof(received.mtype1), (long)PIDclient, MSG_NOERROR) == -1)
    {
      if (errno == EINVAL)
      {
